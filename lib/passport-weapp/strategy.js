...
function WeAppStrategy (options, verify) {
  ...
  this._oauth = new OAuth({
    ...
    isMiniProgram: options.isMiniProgram,
...
WeAppStrategy.prototype.authenticate = function (req, options) {
  ...
  if (req.query && req.query.action === 'codeToSession') {
    ...
    if (code) {
      ...
      return self.response({
        ...
        body: { status: 'error', message: 'code missing' }
      });
    }
  }
  ...
  if (req.query && (req.query.code || req.query.sessionKey)) {
    ...
    function authenticate (data) {
      try {
        var decryptData = self._oauth.decryptMiniProgramData({ sessionKey: data['sessionKey'], iv, encryptedData });
        ...
        if (decryptData && decryptData.watermark && decryptData.watermark.appid === self._oauth.appid) {
          ...
          try {
            if (self._passReqToCallback) {
              ...
              self._verify(req, null, null, profile, null, verified);
            } else {
              self._verify(null, null, profile, null, verified);
            }
          } catch (ex) {
            return self.error(ex);
          }
